...
static struct message_t *msg_pool = NULL;
static int msg_pool_size = 0;

/* Get message from pool or allocate new */
static struct message_t *msg_get(void)
{
	pthread_mutex_lock(&msg_pool_mutex);
	
	if (msg_pool) {
		struct message_t *msg = msg_pool;
		msg_pool = msg->next;
		msg_pool_size--;
...
void msg_put(struct message_t *msg)
{
	if (!msg) return;
...
{
	(void)arg;
	struct message_t *msg = msg_get();
	if (!msg || !msg->data) {
		sys_err("Failed to get message from pool\n");
		if (msg) msg_put(msg);
		return NULL;
	}
...
void *__net_netrcv(void *arg)
{
	...
	struct message_t *msg = msg_get();
	if (!msg || !msg->data) {
		sys_warn("Failed to get message from pool\n");
		if (msg) msg_put(msg);
		return NULL;
	}
...
static void *net_dllbrd(void *arg)
{
	...
	while (g_running) {
		...
		fill_msg_header(&reqbuf->header, MSG_AC_BRD,
			&ac.acuuid[0], ac.random, sequence++);
		...
		sleep((unsigned int)argument.brditv);
	}
...
/* ========================================================================
 * TCP send function for API usage
 * ======================================================================== */
...
int net_send_tcp(const char *ip, int port, const void *data, int len)
{
    ...
    if (sock < 0) {
        sys_err("net_send_tcp: socket failed: %s\n", strerror(errno));
        return -1;
    }
    ...
    if (inet_pton(AF_INET, ip, &addr.sin_addr) <= 0) {
        sys_err("net_send_tcp: invalid IP address: %s\n", ip);
        close(sock);
        return -1;
    }
    ...
    if (ret < 0) {
        sys_warn("net_send_tcp: connect failed to %s:%d: %s\n", ip, port, strerror(errno));
        close(sock);
        return -1;
    }

    ret = send(sock, data, (size_t)len, 0);
    if (ret < 0) {
        sys_err("net_send_tcp: send failed: %s\n", strerror(errno));
        close(sock);
        return -1;
    }
