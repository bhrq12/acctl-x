...
static int check_prerequisites(void)
{
	...
	if (access("/etc/acctl-ac", F_OK) != 0) {
		...
		sys_info("Created /etc/acctl-ac directory\n");
	}

	/* Ensure database file parent directory exists */
	if (strncmp(DBNAME, "/etc/acctl-ac/", 14) == 0) {
		/* Database goes in /etc/acctl-ac - already created above */
	}
...
int main(int argc, char *argv[])
{
	...
	/* 13. Initialize and start HTTP server */
	if (http_server_init() != 0) {
		sys_err("HTTP server initialization failed\n");
		cleanup();
		return -1;
	}
	if (http_server_start() != 0) {
		sys_err("HTTP server start failed\n");
		cleanup();
		return -1;
	}
	sys_info("HTTP server started successfully\n");

	/* 14. Print startup status */
	print_status();
