...
enum {
	...
	MSG_AC_CONFIG,                              /* 1003: AC configuration update */
	MSG_FW_UPGRADE,                             /* 1004: Firmware upgrade trigger */
	MSG_AC_REBOOT,                              /* 1005: AC trigger AP reboot */

	/* AP → AC messages */
	MSG_AP_REG          = 1006,             /* AP registration request */
	MSG_AP_RESP         = 1007,             /* AP response (already registered elsewhere) */
	MSG_AP_STATUS       = 1008,             /* AP status report */
...
static inline uint64_t msg_timestamp_now(void)
{
	...
	return ((uint64_t)tv.tv_sec << 32) | (uint32_t)(tv.tv_usec * 1000);
}
...
fill_msg_header(struct msg_head_t *msg, int msgtype,
	const char *uuid, uint32_t random, uint32_t sequence)
{
	...
	msg->timestamp = msg_timestamp_now();
}
