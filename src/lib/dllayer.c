...
static int __create_sock(int brdcst);
static void __build_brdll(void);
static void __build_rcvll(int bind);
static void __build_sdrll(char *mac);
static void __init_brdcast(int *sock);
static void __dll_buildpkt(char *dmac, char *data, int size);
...
{
	assert(dmac != NULL && (unsigned int)size <= DLL_PKT_DATALEN &&
		data != NULL);
	int ret;
	...
	ret = sendto(dllsdr.sdrsock, dllsdr.sdrpkt,
		(size_t)size + sizeof(struct ethhdr), 0,
...
{
	assert((unsigned int)size <= DLL_PKT_DATALEN && data != NULL);
...
int dll_rcv(char *data, int size, char *src_mac_out)
{
	...
	memcpy(data, dllrcv.rcvpkt + sizeof(struct ethhdr), (size_t)size);
	UNLOCK_RCV();
