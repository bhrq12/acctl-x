 ...
 *                  All data stored in /etc/acctl-ac/ac.json
 *
 *        Version:  2.0
 *        Created:  2026-04-13
...
int db_init(db_t **dbp)
{
    ...
    newdb->dirty_mask = 0;
    newdb->last_save_time = 0;
    newdb->last_modify_time = 0;
...
int db_update_resource(const char *key, const char *value)
{
    ...
    if (!valid) {
        sys_err("db_update_resource: invalid key '%s'\n", key);
        return -1;
    }

    json_object *res = get_or_create_array("resource");
    if (!res) return -1;

    json_object_object_add(res, key,
        json_object_new_string(value ? value : ""));

    db->modified = 1;
    db->dirty_mask |= DIRTY_RESOURCE;
...
int db_ap_list(char *json_buf, int buflen)
{
    ...
    if (!nodes) {
        snprintf(json_buf, buflen, "[]");
        return 0;
    }

    const char *str = json_object_to_json_string_ext(nodes,
        JSON_C_TO_STRING_PLAIN);
    strncpy(json_buf, str, buflen - 1);
    json_buf[buflen - 1] = '\0';
...
{
    if (!db || !db->root || !mac || !json_buf) return -1;
    ...
    const char *str = json_object_to_json_string_ext(node,
        JSON_C_TO_STRING_PLAIN);
    strncpy(json_buf, str, buflen - 1);
    json_buf[buflen - 1] = '\0';
...
int db_ap_update_field(const char *mac, const char *field, const char *value)
{
    ...
    /* Security: Validate MAC address format (XX:XX:XX:XX:XX:XX) */
    size_t mac_len = strlen(mac);
    if (mac_len != 17) {
        sys_err("db_ap_update_field: invalid MAC address length: %zu\n", mac_len);
        return -1;
    }
    for (size_t i = 0; i < mac_len; i++) {
        if (i % 3 == 2) {
            if (mac[i] != ':') {
                sys_err("db_ap_update_field: invalid MAC address format at position %zu\n", i);
                return -1;
            }
        } else {
            if (!((mac[i] >= '0' && mac[i] <= '9') ||
                  (mac[i] >= 'a' && mac[i] <= 'f') ||
                  (mac[i] >= 'A' && mac[i] <= 'F'))) {
                sys_err("db_ap_update_field: invalid MAC address character at position %zu\n", i);
                return -1;
            }
        }
    }
    ...
    if (field_len == 0 || field_len > 64) {
        sys_err("db_ap_update_field: invalid field name length: %zu\n", field_len);
        return -1;
    }
    
    /* Security: Check for dangerous characters in field name */
    for (size_t i = 0; i < field_len; i++) {
        if (field[i] == ';' || field[i] == '|' || field[i] == '`' ||
            field[i] == '$' || field[i] == '(' || field[i] == ')') {
            sys_err("db_ap_update_field: dangerous character in field name\n");
            return -1;
        }
    }
    ...
    if (value && strlen(value) > 1024) {
        sys_err("db_ap_update_field: value too long: %zu\n", strlen(value));
        return -1;
    }
        ...
        if (value) {
            for (size_t i = 0; i < strlen(value); i++) {
                if (value[i] < '0' || value[i] > '9') {
                    if (value[i] != '-' || i != 0) {
                        sys_err("db_ap_update_field: invalid integer value\n");
                        return -1;
                    }
                }
            }
...
int db_group_create(const char *name, const char *description)
{
    ...
    /* Assign next id */
    int max_id = 0;
    size_t len = json_object_array_length(groups);
    for (size_t i = 0; i < len; i++) {
...
int db_group_delete(int group_id)
{
    ...
    json_object_array_del_idx(groups, (size_t)idx, 1);
    db->modified = 1;
    return 0;
}
...
int db_group_list(char *json_buf, int buflen)
{
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        n = snprintf(json_buf + pos, (size_t)(buflen - pos),
    ...
    const char *raw_data)
{
    ...
    /* Assign next id */
    int max_id = 0;
    size_t len = json_object_array_length(alarms);
    for (size_t i = 0; i < len; i++) {
...
int db_alarm_list(char *json_buf, int buflen, int limit)
{
    ...
    if ((size_t)pos + strlen(key) < (size_t)buflen) {
    ...
    for (size_t i = len; i > 0 && count < limit; i--) {
        json_object *al = json_object_array_get_idx(alarms, i - 1);
    ...
    uint32_t file_size, const char *sha256)
{
    ...
    /* Assign next id */
    int max_id = 0;
    size_t len = json_object_array_length(fws);
    for (size_t i = 0; i < len; i++) {
...
int db_firmware_list(char *json_buf, int buflen)
{
    ...
    for (size_t i = len; i > 0; i--) {
        json_object *fw = json_object_array_get_idx(fws, i - 1);
        ...
        int n = snprintf(json_buf + pos, (size_t)(buflen - pos),
...
int db_firmware_delete(const char *version)
{
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        const char *ver = safe_str(fw, "version");
        if (ver && strcmp(ver, version) == 0) {
            json_object_array_del_idx(fws, i, 1);
            db->modified = 1;
            sys_info("Deleted firmware version: %s\n", version);
            return 0;
        }
    }
    return -1;
...
int db_upgrade_start(const char *ap_mac, const char *from_ver, const char *to_ver)
{
    ...
    /* Assign next id */
    int max_id = 0;
    size_t len = json_object_array_length(logs);
    for (size_t i = 0; i < len; i++) {
...
int db_upgrade_finish(const char *ap_mac, const char *status, const char *error_msg)
{
    ...
    size_t len = json_object_array_length(logs);
    for (size_t i = len; i > 0; i--) {
        json_object *log = json_object_array_get_idx(logs, i - 1);
    ...
    char *error_msg, size_t err_len)
{
    ...
    for (size_t i = len; i > 0; i--) {
        ...
        if (from_ver && from_len > 0) strncpy(from_ver, safe_str(log, "from_version"), from_len - 1);
        if (to_ver   && to_len   > 0) strncpy(to_ver,   safe_str(log, "to_version"),   to_len   - 1);
        if (error_msg && err_len > 0) strncpy(error_msg, safe_str(log, "error_message"), err_len - 1);
...
/* ========================================================================
 * Dirty flag management for incremental updates
 * ======================================================================== */
static void mark_dirty(unsigned int mask)
{
    if (db) {
        db->dirty_mask |= mask;
        db->modified = 1;
        db->last_modify_time = time(NULL);
    }
}

static void clear_dirty(unsigned int mask)
{
    if (db) {
        db->dirty_mask &= ~mask;
    }
...
static unsigned int get_and_clear_dirty(void)
{
    if (db) {
        unsigned int mask = db->dirty_mask;
        db->dirty_mask = 0;
        return mask;
    }
    return 0;
...
/* ========================================================================
 * Periodic auto-save with debouncing
 * ======================================================================== */
static void *db_autosave_thread(void *arg)
{
    (void)arg;
    int consecutive_noops = 0;
    ...
    while (1) {
        ...
        time_t now = time(NULL);
        time_t time_since_modify = now - db->last_modify_time;
        time_t time_since_save = now - db->last_save_time;

        if (time_since_modify < 5) {
            sys_debug("Skipping save: last modification was %lds ago (< 5s debounce)\n",
                (long)time_since_modify);
            consecutive_noops++;
            continue;
        }

        if (time_since_save < 10 && consecutive_noops < 3) {
            sys_debug("Skipping save: last save was %lds ago (< 10s throttle)\n",
                (long)time_since_save);
            consecutive_noops++;
            continue;
        }

        if (db_save(db) == 0) {
            db->last_save_time = now;
            consecutive_noops = 0;
            sys_debug("Incremental save completed, dirty_mask cleared\n");
        } else {
            sys_err("Incremental save failed\n");
        }
    }
    return NULL;
}
