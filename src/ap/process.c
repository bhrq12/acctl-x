...
/* MAC to random challenge map (for CHAP) */
...
/* ========================================================================
 * CHAP challenge cleanup thread
 * ======================================================================== */
static void *cleanup_chap_challenges(void *arg)
{
	(void)arg;
	while (1) {
		pthread_mutex_lock(&random_map_lock);
		...
		for (int i = 0; i < LOCAL_AC_MAX; i++) {
			if (random_map[i].mac[0] != 0 &&
				...
				memset(&random_map[i], 0, sizeof(struct mac_random_map_t));
				cleanup_count++;
			}
		}
		if (cleanup_count > 0) {
			sys_debug("Cleaned up %d expired CHAP challenges\n", cleanup_count);
		}
		pthread_mutex_unlock(&random_map_lock);
...
static void *report_apstatus(void *arg)
{
	...
	fill_msg_header((void *)buf, MSG_AP_STATUS, NULL, 0, 0);
	...
	while (1) {
		...
		if (connected) {
			...
			SYSSTAT_UNLOCK();
			if (new_sock < 0) {
				sleep((unsigned int)argument.reportitv);
				continue;
			}
			proto = MSG_PROTO_TCP;
		}
		...
		ret = net_send(proto, sock, (char *)sysstat.dmac, buf, bufsize);
...
static void proc_brd(struct msg_ac_brd_t *msg, int len)
{
	...
	if (!already_reg) {
		...
		map_store_random((char *)msg->header.mac, r1);

		fill_msg_header((void *)resp_buf, MSG_AP_REG,
			msg->header.acuuid, r1, 0);
		...
		chap_fill_msg_md5((void *)resp_buf,
			(int)sizeof(struct msg_ap_reg_t), (int)msg->header.random);

		net_send(MSG_PROTO_ETH, -1, (char *)msg->header.mac,
...
static void proc_reg_resp(struct msg_ac_reg_resp_t *msg, int len)
{
	...
	uint32_t r1 = map_get_random((char *)msg->header.mac);
	if (chap_msg_cmp_md5((void *)msg, (int)sizeof(*msg), r1) != 0) {
...
static void proc_exec_cmd(struct msg_ac_cmd_t *cmd, int __attribute__((unused)) len)
{
...
void ap_msg_proc(void *data, int len, int __attribute__((unused)) proto)
{
...
void init_report(void)
{
	...
	create_pthread(cleanup_chap_challenges, NULL);
	sys_debug("CHAP challenge cleanup thread started\n");
}
