...
static int cmd_aps(int limit)
{
    ...
    printf("{\"aps\":[");
    size_t len = json_object_array_length(nodes);
    int count = 0;
    size_t i;
...
static int cmd_alarms(int limit)
{
    ...
    int count = 0;
    size_t i;
    for (i = len; i > 0 && count < limit; i--) {
        json_object *a = json_object_array_get_idx(alarms, i - 1);
        if (i < len) printf(",");
...
static int cmd_stats(void)
{
    ...
    size_t ap_total = 0, ap_online = 0, alarms = 0, groups = 0;

    json_object *nodes;
    if (json_object_object_get_ex(root, "nodes", &nodes) &&
        json_object_is_type(nodes, json_type_array)) {
        ap_total = json_object_array_length(nodes);
        size_t nlen = ap_total;
    ...
    if (json_object_object_get_ex(root, "alarm_events", &ev) &&
        ...
        size_t alen = json_object_array_length(ev);
    ...
    printf("{\"ap_total\":%zu,\"ap_online\":%zu,\"alarms\":%zu,\"groups\":%zu}\n",
